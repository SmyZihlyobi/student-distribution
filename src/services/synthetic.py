...
def generate_students(num_students: int, teams: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    team_skills = {
        "Cloud": "Навыки: AWS, Azure, Docker, Kubernetes, Terraform",
        "ML": "Навыки: Python, TensorFlow, PyTorch, Scikit-learn, Pandas",
        "Data": "Навыки: SQL, Data Mining, Hadoop, ETL",
        "DevOps": "Навыки: Linux, Docker, Jenkins, CI/CD",
        "Web": "Навыки: HTML, CSS, JavaScript, React, Angular, Vue.js",
    }

    students_prompt = f"""Сгенерируй {num_students} студентов в CSV формате. Прикрепи их к командам. 
    Укажи навыки в зависимости от команды.
    ...
    resume_text должен содержать навыки студента на русском языке, в зависимости от команды, куда он прикреплен. Укажи только подходящие навыки.
    """
...
def generate_team_compatibility(teams: List[Dict[str, str]], projects: List[Dict[str, str]],
                               students: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    formatted_teams = "\n".join([
        f"{team['id']}: {team['team_name']} ({team['specialization']}). Skills: {', '.join(team_skills.get(team['id'], []))}"
        for team in teams])
    formatted_projects = "\n".join(
    ...
    The compatibility score should be a number between 0 and 1, representing how well suited a team is for a project. Base the score on the team's specialization, the project's description, stack, required roles, AND the skills of the students in each team. A higher score means a better fit. Each possible team/project combination must have an entry. All data and descriptions are in English. The compatibility score should range from 0.0 to 1.0. Do not leave anything out.
    """
...
def update_ids_in_data(data: Dict[str, List[Dict]], next_ids: Dict[str, int]) -> Dict[str, List[Dict]]:
    ...
    for data_type, items in data.items():
        ...
        for item in items:
            ...
            if data_type == 'teams':
                updated_item['id'] = str(next_ids_copy['teams'])
                next_ids_copy['teams'] += 1
            elif data_type == 'students':
                updated_item['id'] = str(next_ids_copy['students'])
                next_ids_copy['students'] += 1
            elif data_type == 'projects':
                updated_item['id'] = str(next_ids_copy['projects'])
                company_id = (int(updated_item['id']) - 1) % 10 + 1
                updated_item['company_id'] = str(company_id)
                next_ids_copy['projects'] += 1
    ...
    for item in updated_data.get('students', []):
        student_id_mapping[item['id']] = item['id']
